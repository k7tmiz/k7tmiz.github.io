...
- 复习本轮：作为独立入口复习当前 A4（默认打乱顺序，可恢复顺序；支持左右滑动/拖拽标记：右滑=已掌握、左滑=不会，手机/平板触摸与电脑鼠标拖拽均可；未达提交条件会自动平滑回中；支持快速甩动提交；复习完成默认自动关闭弹窗，可在设置关闭；可选开启“点击卡片翻面”，开启后：点单词发音、点空白翻面、背面点任意处翻回）
